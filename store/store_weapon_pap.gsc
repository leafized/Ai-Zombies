    ...
    for(;;)
    {
        ...
        if(isDefined(level.papMachine))
        {
            if(distance(self.origin, level.papMachine.origin ) <100)
            {
                ...
                self setLower("pap", "Press ^3[{+activate}] ^7for ^5Pack-A-Punch ^7(^2$^3"+level.store_item_price_pap+"^7)",undefined,0);
                if(self usebuttonpressed() && self.isPacked[self GetCurrentWeapon()] != 9)
                {
                    self tryBuying(self GetCurrentWeapon(), "pap", level.store_item_price_pap);//tryBuying(item,type,price,parent_entity)
                    wait 1;
                }
            }
